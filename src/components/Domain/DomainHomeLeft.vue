  ...
  <div class="container">
    ...
    <el-card class="box-card" :body-style="elcardbody">
      ...
      <p v-show="isOpenService || index < max" v-for="(item, index) in list" :key="index">
        <span :style="cardspanleft">{{ item.label }}</span
        ><span :style="cardspanright">{{ item.value }}</span>
      </p>
      <p :style="arrow">
...
export default {
  data () {
    return {
      ...
      cardspanright: { color: '#7F7F7F', width: '50%', display: 'inline-block', textAlign: 'right' },
