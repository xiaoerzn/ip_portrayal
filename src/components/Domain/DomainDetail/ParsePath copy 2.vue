...
export default {
  ...
  methods: {
    ...
    toggleShowNodes (chart, params) {
      ...
      var srcLinkName = params.name; var // 节点名字
        serieLinks = options.series[seriesIndex].links; var // option 的links
        serieData = options.series[seriesIndex].data; var // option 的data
      ...
      if (open) {
        ...
        if (serieLinkArr.length) {
          ...
          for (var j = 0; j < serieLinkArr.length; j++) {
            ...
            if (serieDataMap.has(serieDataMap[j])) {
              // console.log("has")
